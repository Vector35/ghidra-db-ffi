...
public class Schema {
	...
	private static Integer[] arrayUtilsToObject(final int[] array) {
		if (array == null) {
			return null;
		...
		for (int i = 0; i < array.length; i++) {
			result[i] = Integer.valueOf(array[i]); // Autoboxing or explicit wrapping
		}
		return result;
	}

	private static byte[] arrayUtilsToPrimitive(final Byte[] array) {
		if (array == null) {
			return null;
		} else if (array.length == 0) {
			return new byte[0];
		...
		for (int i = 0; i < array.length; i++) {
			result[i] = array[i].byteValue(); // Autoboxing or explicit wrapping
		}
		return result;
	}
	...
	public Schema(int version, Field keyField, String keyName, Field[] fields,
		...
		try {
			initializeSparseColumnSet(arrayUtilsToObject(sparseColumns));
		}
	...
	byte[] getEncodedFieldTypes() {
		...
		return arrayUtilsToPrimitive(encodedDataList.toArray(Byte[]::new));
	}
