...
// TODO: May need to make repair an option (may not have write privilege)
			...
			if (isVersioned) {
				...
				for (int i = versions.length - 1; i >= 0; i--) {
					if (versions[i] >= minVersion) {
						if (repair) {
							...
							LogWarn(dbDir + ": removing unexpected version file: " + f);
							f.delete();
						}
					}
						...
						LogWarn(dbDir + ": missing version file " + (minVersion - 1));
						filesOrphaned = true;
						break;
					}
				}
				...
				for (int i = changes.length - 1; i >= 0; i--) {
					if (changes[i] >= minChangeVer) {
						if (repair) {
							...
							LogWarn(dbDir + ": removing unexpected change file: " + f);
							f.delete();
						}
					}
						...
						LogWarn(dbDir + ": missing change file " + (minVersion - 1));
						filesOrphaned = true;
						break;
					}
				}
				...
				if (minChangeVer > minVersion) {
					LogWarn(dbDir + ": missing change files prior to " + minChangeVer);
					minVersion = minChangeVer;
					filesOrphaned = true;
				}
				if (repair && filesOrphaned) {
					LogWarn(dbDir + ": versions prior to " + minVersion +
	...
	private int[] getFileVersions(ArrayList<String> fileList) {
		...
		while (iter.hasNext()) {
			...
			if (ix1 < 0 || ix2 < ix1) {
				LogError(dbDir + ": bad file name: " + fname);
				continue;
			}
		...
		public void versionCreated(int version, String comment, long checkinId)
			...
			synchronized (syncObject) {
				if (currentVersion != (version - 1)) {
					LogError(dbDir + ": unexpected version created (" + version +
				...
				if (version != currentVersion) {
					LogError(dbDir + ": Unexpected version found (" + currentVersion +
						"), expected " + version);
				}
