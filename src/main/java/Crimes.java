...
should we explicitly specify utf8 for all string conversions?
...
public class Crimes {
    ...
    @CStruct("ghidra_schema_t")
    interface CGhidraSchema extends PointerBase {
        CGhidraSchema addressOf(int index);
        ...
        @CField("field_types")
        CIntPointer fieldTypes();

        @CField("field_types")
        void fieldTypes(CIntPointer fieldTypes);
        ...
        @CField("packed_field_names")
        CCharPointer packedFieldNames();

        @CField("packed_field_names")
        void packedFieldNames(CCharPointer packedFieldNames);
    }
    ...
    private static DBHandle getDbHandleFromId(int dbId, boolean checkOpen) throws Exception {
        DBHandle handle;
        synchronized (g_dbInstances) {
            handle = g_dbInstances.get(dbId);
        }
    ...
    private static Schema schemaFromCGhidraSchema(CGhidraSchema cSchema) throws UnsupportedFieldException {
        int version = cSchema.version();

        CIntPointer fieldTypesPtr = cSchema.fieldTypes();
        byte encodedKeyFieldType = (byte) fieldTypesPtr.read(0);
        int fieldCount = cSchema.fieldCount();
        byte[] encodedFieldTypes = new byte[fieldCount - 1];
        for (int i = 1; i < fieldCount; i++) {
            encodedFieldTypes[i - 1] = (byte) fieldTypesPtr.read(i);
        }

        String packedFieldNames = CTypeConversion.toJavaString(cSchema.packedFieldNames());

        return new Schema(version, encodedKeyFieldType, encodedFieldTypes, packedFieldNames);
    }

    @CEntryPoint(name = "FFI_open_db_handle")
    public static boolean openDbHandle(@CEntryPoint.IsolateThreadContext long isolateId, @CConst CCharPointer cFileName,
            CIntPointer dbId) {
    ...
    public static CCharPointerPointer getTableNames(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            CIntPointer count) {
        try {
            DBHandle handle = getDbHandleFromId(dbId);
    ...
    public static boolean get_table_schema_version(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cTableName, CIntPointer version) {
        try {
    ...
    public static boolean get_table_col_count(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cTableName, CIntPointer count) {
        try {
            final String tableName = CTypeConversion.toJavaString(cTableName);
    ...
    public static boolean get_table_col_type(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cTableName, int col, CIntPointer type) {
        try {
            final String tableName = CTypeConversion.toJavaString(cTableName);
    ...
    public static boolean get_table_col_name(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cTableName, int col, CCharPointerPointer name) {
        try {
            final String tableName = CTypeConversion.toJavaString(cTableName);
    ...
    public static boolean get_table_row_count(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cTableName, CIntPointer count) {
        try {
            final String tableName = CTypeConversion.toJavaString(cTableName);
    ...
    public static boolean get_table_records(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cTableName, CGhidraRecordPointerPointer records, CIntPointer count) {
        ...
        try {
            ...
            CGhidraRecordPointer recordArray = UnmanagedMemory
                    .calloc(SizeOf.get(CGhidraRecordPointer.class) * recordCount);
            ...
            for (int recordNum = 0; recordNum < recordCount; recordNum++) {
                ...
                CGhidraFieldPointer fieldArray = UnmanagedMemory
                        .calloc(SizeOf.get(CGhidraFieldPointer.class) * fieldCount);
    ...
    public static boolean get_buffer(@CEntryPoint.IsolateThreadContext long isolateId, int dbId, int bufferId,
            CCharPointerPointer bufferData, CIntPointer bufferSize) {
        try {
            DBHandle handle = getDbHandleFromId(dbId);
    ...
    public static boolean get_buffer(@CEntryPoint.IsolateThreadContext long isolateId, int dbId, int bufferId,
            int shadowBufferId, CCharPointerPointer bufferData, CIntPointer bufferSize) {
        try {
            DBHandle handle = getDbHandleFromId(dbId);
    ...
    public static boolean free_records(@CEntryPoint.IsolateThreadContext long isolateId,
            CGhidraRecordPointer recordArray, int count) {
        try {
    ...
    public static boolean free_fields(@CEntryPoint.IsolateThreadContext long isolateId, CGhidraFieldPointer fieldArray,
            int count) {
        try {
            for (int i = 0; i < count; i++) {
    ...
    public static void free_string_list(@CEntryPoint.IsolateThreadContext long isolateId, CCharPointerPointer p,
            int count) {
        for (int i = 0; i < count; i++) {
    ...
    @CEntryPoint(name = "FFI_create_empty_dbhandle")
    public static boolean createEmptyDBHandle(@CEntryPoint.IsolateThreadContext long isolateId, CIntPointer dbId) {
        try {
    ...
    @CEntryPoint(name = "FFI_create_table")
    public static boolean createTable(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cTableName, @CConst CGhidraSchema cSchema, @CConst CIntPointer cIndexedColumns,
            int indexedColumnCount) {
        try {
            // TODO: do we need to allow specifying sparse columns?
            final String tableName = CTypeConversion.toJavaString(cTableName);
            int[] indexedColumns = new int[indexedColumnCount];
            for (int i = 0; i < indexedColumnCount; i++) {
                indexedColumns[i] = cIndexedColumns.read(i);
            }
            DBHandle handle = getDbHandleFromId(dbId);
            long txId = handle.startTransaction();
            handle.createTable(tableName, schemaFromCGhidraSchema(cSchema), indexedColumns);
    ...
    @CEntryPoint(name = "FFI_pack_database")
    public static boolean packDatabase(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cOutputFilePath) {
        try {
            DBHandle handle = getDbHandleFromId(dbId);
            ...
            File tmpFile = null;
            try {
                tmpFile = File.createTempFile("ghidra-export-pack", ".tmp");
                tmpFile.delete();
                handle.saveAs(tmpFile, false, null);
                // TODO: support exporting type archives
                // TODO: allow specifying item name
                // TODO: monitor progress callbacks
                try (InputStream itemIn = new BufferedInputStream(new FileInputStream(tmpFile))) {
                    ItemSerializer.outputItem("exported-item", "Program", 0,
                            tmpFile.length(), itemIn, outputFile, null);
                }
                success = true;
            } finally {
                tmpFile.delete();
    ...
    @CEntryPoint(name = "FFI_free_schema")
    public static void freeSchema(@CEntryPoint.IsolateThreadContext long isolateId, CGhidraSchema cSchema) {
        try {
            UnmanagedMemory.free(cSchema.fieldTypes());
            UnmanagedMemory.free(cSchema.packedFieldNames());
            UnmanagedMemory.free(cSchema);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    @CEntryPoint(name = "FFI_create_buffer")
    public static boolean createBuffer(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
            @CConst CCharPointer cBuffer, int cBufferSize, CIntPointer bufId) {
        try {
            DBHandle handle = getDbHandleFromId(dbId);
            long txId = handle.startTransaction();
            DBBuffer dbBuffer = handle.createBuffer(cBufferSize);

            byte[] byteArr = new byte[cBufferSize];
            ByteBuffer byteBuf = CTypeConversion.asByteBuffer(cBuffer, cBufferSize);
            byteBuf.get(byteArr);

            dbBuffer.put(0, byteArr);
            handle.endTransaction(txId, true);
            bufId.write(dbBuffer.getId());
            return true;
        } catch (Exception e) {
            // TODO: need to end transaction in case of error
            e.printStackTrace();
    ...
    public static Field cFieldToField(CGhidraFieldPointer cField) throws Exception {
        GhidraFieldType fieldType = GhidraFieldType.fromCValue(cField.fieldType());
        switch (fieldType) {
            case GhidraFieldType.BYTE_TYPE:
                return new ByteField(cField.byteValue());
            case GhidraFieldType.SHORT_TYPE:
                return new ShortField(cField.shortValue());
            case GhidraFieldType.INT_TYPE:
                return new IntField(cField.intValue());
            ...
            case GhidraFieldType.BINARY_OBJ_TYPE: {
                ...
                dataBuf.get(data);
                return new BinaryField(data);
            }
            case GhidraFieldType.BOOLEAN_TYPE:
                return new BooleanField(cField.booleanValue());
            default:
                throw new Exception("Unhandled field type " + fieldType.toString());
        }
    }
    ...
    public static boolean writeRecordToTable(@CEntryPoint.IsolateThreadContext long isolateId, int dbId,
        ...
        try {
            ...
            CGhidraFieldPointer cFields = cRecord.fields();

            CGhidraFieldPointer cKeyField = cFields.addressOf(0);
            Field keyField = cFieldToField(cKeyField);

            DBRecord record = tableSchema.createRecord(keyField);
